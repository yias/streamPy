...
def computeTriggerValue(mouseInput, previousTrigger = 0):
	...
	if (mouseInput[1] > threshold):
		x_dir = 1.0
	if (mouseInput[1] < -1*threshold):
		x_dir = -1.0
	if (mouseInput[0] > threshold):
		y_dir = 1.0
	if (mouseInput[0] < -1*threshold):
		y_dir = -1.0
	...
	while(True):
		try:
			...
			if publish:
				...
				trigger_value = computeTriggerValue(msg_tr, trigger_value)
					
				weight_value = -1

				# throwing the data in a json object
				data=json.dumps({"translation": msg_tr, "rotation": msg_rot, "button": msg_btn, "command": trigger_value, "weight": weight_value})
